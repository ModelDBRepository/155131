%
%	plot_int_2: produce several plots
%	1..3 neurons
...
function plot_int_2(FN, sim, nn_inputs, LW, nn_pars, off)
...
maxp = min(-nn_inputs(1,off:end)-nn_inputs(2,off:end));
ylabel('I_s');
axis([0,sim.T_upd-off,maxp,minp]);
set(gca,'Visible','off');
...
hold on;
if (1==0),
	text(sim.T_upd-off -40,minp-0.2,'I_s','FontSize',[16]);
else
	%
	% integrated input
	%
	...
	text(sim.T_upd-off -80,minp-0.2,inp,'FontSize',[12]);
end;
...
plot([sim.T_upd-50-off, sim.T_upd],[maxp+0.1,maxp+0.1],'Linewidth',2);
text(sim.T_upd-50-off,maxp-0.2,'50ms','FontSize',[12]);
